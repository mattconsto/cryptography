...
def reformat(plain, cipher):
	...
	for index, character in enumerate(plain):
		while index + offset < len(cipher) and not re.match(r"[a-zA-Z]", cipher[index + offset]):
			output += cipher[index + offset]
			offset += 1

		if index + offset < len(cipher) and re.match(r"[A-Z]", cipher[index + offset]):
